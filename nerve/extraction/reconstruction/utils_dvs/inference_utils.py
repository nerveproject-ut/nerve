...
class ImageWriter:
    ...
    Also writes the image timestamps        with CudaTimer('Intensity rescaling'):
 into a text file.
    """
    ...
    def __init__(self, options):
        ...
        self.type = str(options.output_type)
        assert self.type == 'img' or self.type == 'lossy-video' or self.type == 'lossless-video'
        self.store_video = self.type != 'img'

        #self.num_bins_to_show = options.num_bins_to_show
        self.num_bins_to_show = -1
        ...
        if self.output_folder:
            ...
            if not self.store_video:
                ...
                # new code, let's write LOSSLESS video(s) instead of images. -Pietro
                assert options.fixed_duration, "You can save videos only using the fixed_duration option"

                crf_value = '0' if self.type == 'lossless-video' else '23'

                self.rec_frame_writer = VideoWriter_x264(join(self.output_folder, "reconstruction.mp4"), 
                                                         fps=options.fps, in_pix_fmt='gray', output_args={'preset': 'fast', 'crf': crf_value, 'loglevel': 'quiet' })
                if self.save_events:
                    self.events_frame_writer = VideoWriter_x264(join(self.output_folder, "events.mp4"), 
                                                         fps=options.fps, in_pix_fmt='bgr24', output_args={'preset': 'fast', 'crf': crf_value, 'loglevel': 'quiet' })

        else:
            print('Will not write images or videos to disk.')

    def __call__(self, img, stamp=None, events=None):
        if not self.output_folder:
            return

        if self.store_video:
            self.rec_frame_writer.write(img)
            if self.save_events:
                event_preview = make_event_preview(events, mode=self.event_display_mode,
                                                num_bins_to_show=self.num_bins_to_show)
                self.events_frame_writer.write(event_preview)
        else:
            self.frames_written += 1
            file_title = self.frames_written

            if self.save_events and events is not None:
                event_preview = make_event_preview(events, mode=self.event_display_mode,
                                                num_bins_to_show=self.num_bins_to_show)
                cv2.imwrite(join(self.event_previews_folder,
                                'events_{:06d}.png'.format(file_title)), event_preview)

            #print("frame: {} -> {} \n events: {} -> {}".format(img.shape, img.dtype, event_preview.shape, event_preview.dtype))

            cv2.imwrite(join(self.output_folder, self.dataset_name,
                            'frame_{:06d}.png'.format(file_title)), img)
    ...
    def __cleanup__(self):
        if self.output_folder:
            if not self.timestamps_file is None:
