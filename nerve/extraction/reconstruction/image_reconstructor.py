...
from utils_dvs.inference_utils import CropParameters, EventPreprocessor, IntensityRescaler, ImageFilter, ImageDisplay, ImageWriter, UnsharpMaskFilter
from utils_dvs.inference_utils import upsample_color_image, merge_channels_into_color_image  # for color reconstruction
from utils_dvs.util import robust_min, robust_max
...
class ImageReconstructor:
    def __init__(self, model, height, width, options):
    ...
    def update_reconstruction(self, event_tensor, stamp=None):
        with torch.no_grad():
            ...
            for channel in events_for_each_channel.keys():
                f, s = self.model(events_for_each_channel[channel],
                                                            self.last_states_for_each_channel[channel])
                
                # Pietro, 19/10/23
                # So, for some unknown reason it happens from time to time to feed the model with "normal" data, and to get back NaN values..
                # So far this has been seen only with DAVIS camera (low resolution), when the number of events is close to 0.
                # This appears to be a problem in the model, not about data themselves.
                # in these occasion, let's just go on like if those data weren't feed to the model, so let's store a frame equal to the previous one.
                # If you don't like this policy, feel free to remove the next statements.
                if torch.isnan(f).sum() == 0:
                    new_predicted_frame, states = f, s
                    self.last_correct_frame = f
                    self.last_correct_state = s
                else:
                    warnings.warn("The given tensor of events, for some reason, did't behave correctly.. Using last one instead.") 
                    new_predicted_frame =self.last_correct_frame
                    states = self.last_correct_state
            ...
            with Timer("Writing frames to file"):
                self.image_writer(out, stamp, events=events)
