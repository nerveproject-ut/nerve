...
class BaseTrainer:
    ...
    def _setup_ddp(self, rank, world_size):
        ...
        # Allow forcing GLOO backend via environment variable (useful when NCCL has issues)
        force_gloo = os.environ.get('FORCE_GLOO_BACKEND', '0') == '1'
        if force_gloo:
            backend = 'gloo'
            self.console.info(f'DDP settings: RANK {rank}, WORLD_SIZE {world_size}, DEVICE {self.device}, BACKEND: gloo (forced)')
        else:
            backend = 'nccl' if dist.is_nccl_available() else 'gloo'
            self.console.info(f'DDP settings: RANK {rank}, WORLD_SIZE {world_size}, DEVICE {self.device}, BACKEND: {backend}')
        
        dist.init_process_group(backend, timeout=timedelta(seconds=3600), rank=rank, world_size=world_size)
