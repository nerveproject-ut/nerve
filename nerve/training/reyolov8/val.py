...
class EventVideoDetectionValidator(BaseValidator):
    ...
    def __init__(self, video_config = None, dataloader=None, save_dir=None, pbar=None, logger=None, args=None):
        ...
        # Note: self.args is already set by super().__init__() with proper defaults (including conf=0.001)
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            LOGGER.info("Loading model for standalone validation...")
            ...
            # Use existing dataloader if provided, otherwise create one
            if self.dataloader is None:
                LOGGER.info(f"Creating dataloader for '{self.args.split}' split (sequential mode - full sequences)...")
                self.dataloader = self.get_dataloader(self.data.get(self.args.split), self.args.batch, rank=-1, load="sequential", speed=self.args.speed, zero_hidden=self.args.zero_hidden)
            else:
                LOGGER.info(f"Using provided dataloader with {len(self.dataloader)} batches")
            ...
            LOGGER.info("Warmup complete. Starting validation loop...")
    ...
    def postprocess(self, preds):
        """
        Post-process predictions using NMS.
        
        Passes nc explicitly to preserve extra channels (like distance) 
        that would otherwise be misinterpreted as additional classes.
        
        If self.args.classes is set, only predictions for those classes are kept.
        This is useful for zero-shot evaluation when the model has more classes
        than your dataset (e.g., pretrained on pedestrian+two-wheeler but your
        dataset only has pedestrian annotations).
        """
        # Get classes filter if specified
        classes_filter = getattr(self.args, 'classes', None)
        
        preds = ops.non_max_suppression(preds,
                                        self.args.conf,  # conf threshold
                                        self.args.iou,   # iou threshold
                                        classes=classes_filter,  # Filter by class indices
                                        labels=self.lb,
                                        multi_label=True,
                                        agnostic=False,
                                        max_det=self.args.max_det,
                                        nc=self.nc)  # Preserve distance channel
        return preds
    ...
    def get_dataloader(self, dataset_path, batch_size,  mode='val', rank=0, load = 'sequential', speed = False, zero_hidden = False):
        ...
        select_channels = getattr(self.args, 'select_channels', None)
        return build_video_val_standalone_dataloader(self.args, self.video_config, batch_size,dataset_path,rank=rank, mode = load, speed = speed, zero_hidden = zero_hidden, select_channels=select_channels)[0]
...
def val(cfg=DEFAULT_CFG,use_python=False):
 ...
 def parse_opt(known=False):
    ...
    parser.add_argument('--classes', nargs='+', type=int, default=None,
                        help='Filter predictions by class indices (e.g., --classes 0 for pedestrian only)')
