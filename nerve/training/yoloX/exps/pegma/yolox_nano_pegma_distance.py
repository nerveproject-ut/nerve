...
# Try to import Distance evaluator for distance-specific metrics
try:
    import sys
    deep_dir = yolox_root.parent
    if str(deep_dir) not in sys.path:
        sys.path.insert(0, str(deep_dir))
    from distance_coco_evaluator import Distance_COCO_Evaluator
...
# Import distance-aware MosaicDetection
...
class Exp(BaseExp):
    ...
    def __init__(self):
        ...
        # Final path will be: output_dir/exp_name/ (e.g., runs/yolox_distance/)
    ...
    def get_model(self, sublinear=False):
        ...
        if getattr(self, "model", None) is None:
            ...
            try:
                # Add deep directory to path for custom_yolo import
                deep_dir = yolox_root.parent
                if str(deep_dir) not in sys.path:
                    sys.path.insert(0, str(deep_dir))
