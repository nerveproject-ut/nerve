...
try:
    ...
    def _add_prefix(metrics, prefix, separator):
        if not prefix:
            return metrics
    ...
    def _flatten_dict(params, delimiter="/"):
    ...
    def _sanitize_callable_params(params):
        return {k: v if not callable(v) else str(v) for k, v in params.items()}
...
class WandbLogger(Logger):
    ...
    def _get_public_run(self):
        if self._public_run is None:
            ...
            # Use public properties (entity, project, id) instead of private attributes
            # which were removed in newer versions of wandb
            ...
            # Check if wandb is in offline mode or disabled (entity/project will be 'dummy')
            if entity == 'dummy' or project == 'dummy' or entity is None or project is None:
                return None
            ...
            try:
                ...
                # Run not found - wandb may be in offline mode or run not synced yet
                rank_zero_warn(f"Could not find wandb run at {runpath}: {e}")
                return None
    ...
    def _num_logged_artifact(self):
        ...
        try:
            ...
            rank_zero_warn(f"Could not get logged artifacts: {e}")
            return 0
    ...
    def _rm_but_top_k(self, top_k: int):
        ...
        # Skip if wandb is in offline mode or run not found
        if public_run is None:
            return
