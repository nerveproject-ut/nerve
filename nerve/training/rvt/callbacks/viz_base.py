...
# Type alias for supported loggers
SupportedLogger = Union[WandbLogger, LocalLogger]
...
class VizCallbackBase(Callback):
    ...
    def on_train_batch_end(
        ...
        logger: Optional[SupportedLogger] = trainer.logger
        assert isinstance(logger, (WandbLogger, LocalLogger)), f"Expected WandbLogger or LocalLogger, got {type(logger)}"
    ...
    def ev_repr_to_img(x: np.ndarray):
        ...
        # Handle odd channel counts (e.g., when radar channel is fused)
        # Only use even number of channels for visualization (ignore extra radar channel)
        if ch > 1 and ch % 2 != 0:
            # Assume the last channel is the radar/extra channel, exclude it
            x = x[..., :-1, :, :]
            ch = ch - 1
        
        # Handle edge case where we have only 1 or 0 channels after removal
        if ch < 2:
            # Fallback: just create a grayscale image from the first channel
            img = np.asarray(x[..., 0, :, :], dtype='float32')
