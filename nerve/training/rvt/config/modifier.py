...
def dynamically_modify_train_config(config: DictConfig):
    with open_dict(config):
        ...
        assert dataset_name in {'gen1', 'gen4', 'pegma'}
        ...
        if mdl_name == 'rnndet':
            ...
            if backbone_name in {'MaxViTRNN', 'maxvit_rnn'}:
            ...
            if dataset_name == 'gen1':
                ...
                # PEGMA: use num_classes from config if set, otherwise default to 1 (person)
                num_classes = mdl_cfg.head.get('num_classes', 1)
            else:
                num_classes = mdl_cfg.head.get('num_classes', 1)
